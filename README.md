...
- **LLM-саммари** — автоматический анализ лекций через OpenRouter (GPT-4o-mini)
...
Отправляет текстовые файлы в LLM (GPT-4o-mini через OpenRouter) для создания развёрнутого содержания с таймкодами, определениями и резюме.

```bash
# Текст + саммари для одного видео
python download.py URL --text --summary --ru

# Целый плейлист
python download.py PLAYLIST_URL --text --summary --ru
```

Сохраняет в `downloads/summaries/Название.txt` с:
- Детальные таймкоды каждого смыслового блока
- Описание понятий, теорем, формул
- Ключевые термины по каждому блоку
- Общее резюме лекции
- Рекомендации к следующей лекции
...
- `--ru` подхватит прокси из `.env` автоматически
- `--summary` подхватит API ключ из `.env`
